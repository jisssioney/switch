...
FORWARD_STP_STORM_CONFIG_KEYS = frozenset(
...
def validate_forward_stp_storm_config(config):
    if (
        not isinstance(config, dict)
        or frozenset(config) != FORWARD_STP_STORM_CONFIG_KEYS
    ):
        raise InvalidInput("bad config")
    ...
    if not all(_is_int(value) and value > 0 for value in values):
        raise InvalidInput("bad storm value")
...
def forward_stp_storm(
    ...
    allowed = {}  # (port, vlan, 类别) -> [放行时刻]
    moves = {}  # (vlan, mac) -> [最后学习端口, [迁移时刻]]
    blocked = {}  # (port, vlan) -> 封锁截止时刻
    ...
    for item in events:
        ...
        if state in ("learning", "forwarding"):
            block_key = (port_name, vlan)
            until = blocked.get(block_key)
            if until is not None and t >= until:  # t>=截止解除
                del blocked[block_key]
                until = None
            suppressed = until is not None  # 封锁帧不检测
            if not suppressed:
                if dst == BROADCAST_MAC:
                    category = "broadcast"
                elif int(dst[:2], 16) & 1:
                    category = "multicast"
                elif (vlan, dst) not in fdb:
                    category = "unknown"
                else:
                    category = None  # 命中单播不抑制
                if category is not None:
                    key = (port_name, vlan, category)
                    times = allowed.setdefault(key, [])
                    times[:] = [seen for seen in times if t - seen < window]
                    if len(times) >= limits[category]:
                        suppressed = True
                    else:
                        times.append(t)
            if not suppressed:
                move = moves.get((vlan, src))
                if move is None:
                    moves[(vlan, src)] = [port_name, []]
                elif move[0] != port_name:
                    move_times = move[1]
                    move_times[:] = [
                        seen for seen in move_times if t - seen < window
                    ]
                    move_times.append(t)
                    if len(move_times) >= move_limit:
                        blocked[block_key] = t + hold
                        suppressed = True
                    else:
                        move[0] = port_name
            if suppressed:  # 抑制丢弃：不学习、不发送，按无出口计 drop
                port_stats[port_name]["drop"] += 1
                vlan_stats[vlan]["drop"] += 1
                results.append({"t": t, "action": "drop", "ports": []})
                continue
