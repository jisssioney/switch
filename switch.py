...
def validate_acl_config(config):
    ...
    for rule in rules:
        ...
        if ethertype is not None and (
            not _is_int(ethertype) or not 0 <= ethertype <= 65535
        ):
            raise InvalidInput("bad acl ethertype")
        if priority is not None and (
            not _is_int(priority) or not 0 <= priority <= 7
        ):
        ...
        elif not _valid_vlan_id(to_vlan):
            raise InvalidInput("bad acl to_vlan")
        parsed.append(
            {
                "src": src,
...
def forward_acl(
    ...
    for item in events:
        ...
        port_stats[port_name]["rx"] += 1
        if tag is None:
            orig_vlan = by_name[port_name]["pvid"]
            rejected = False
        else:
            orig_vlan = tag
            ingress = by_name[port_name]
            rejected = (
                ingress["mode"] == "access"
                or orig_vlan not in ingress["allowed"]
            )
        if rejected:  # VLAN 准入拒绝：不学习、不计 VLAN
            port_stats[port_name]["drop"] += 1
            results.append(
            ...
            continue
        # 原 VLAN 准入后，以有效 VLAN 及原字段匹配 ACL（每帧仅匹配一次）
        rule = acl_match(orig_vlan, src, dst, ethertype, priority)
        action_name = rule["action"] if rule is not None else "allow"
        if action_name == "drop" or (
            action_name == "remark"
            and rule["to_vlan"] not in by_name[port_name]["allowed"]
        ):
            # ACL drop（含 remark 目标 VLAN 未准入）：不学习、不计风暴、
            # 不镜像、不发送；计入站口与原 VLAN 的 rx、drop
            port_stats[port_name]["drop"] += 1
            vlan_stats[orig_vlan]["rx"] += 1
            vlan_stats[orig_vlan]["drop"] += 1
            results.append(
                {"t": t, "action": "drop", "ports": [], "mirrors": []}
            )
            continue
        vlan = orig_vlan
        if action_name == "remark":
            vlan = rule["to_vlan"]
        # 放行（含改写后准入）：入端口命中 sources 则复制入站原始标签
        ingress_copy = None
        if direction in ("ingress", "both") and port_name in source_set:
            ingress_copy = mirror_entry("ingress", tag, port_name, t)
        vlan_stats[vlan]["rx"] += 1
